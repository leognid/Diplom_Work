...
class VK_request:
    ...
    def get_photo_info(self):
      ...
      if response.status_code == 200:
        print(f" Код {response.status_code} Серевер работает успешно")
      else:
        return
...
class Yandex:
    ...
    def create_folder(self, folder_name):
        ...
        if requests.get(url, headers=self.headers, params=params).status_code != 200:
            ...
            print(f'\nПапка {folder_name} успешно создана на Яндекс диске диска\n')
        else:
            print(f'\nПапка {folder_name} уже существует. Фото с одинаковыми именами не будут скопированы\n')
        return folder_name    
